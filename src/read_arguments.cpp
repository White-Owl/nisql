...
static void show_usage() {
	cout << "Usage: nisql conn query file [options]\n"
	     ...
	     << "\tSETS=text[,text]\tName of output files for multiple resultests\n"
	     << "Options for CSV format:\n"
	     << "\tHDR=ON|OFF\tPrint column headers (ON)\n"
	     << "\tDELIM=text\tSet field deilimiter, use \"\\t\" for tabulation\n"
	     << "\tQUOTE=ON|OFF\tQuote text fields always (Off)\n"
	     << "\tSETS=text[,text]\tName of output files for multiple resultests\n"
	     << "Options for XML format:\n"
	     << "\tSTREAM=ON|OFF\tWrite XML into one stream or format it (ON)\n"
	     << "\tTAGS=ON|OFF\tPrint fileds as tags (for ON) or values (for OFF) (default ON)\n"
	     << "\tROOT=text\tName for the XML's root (default 'data')\n"
	     << "\tSETS=text[,text]\tName(s) of the recordset(s), for multiset queries\n"
	     << "\tROWS=text[,text]\tName(s) for row element in recordset(s)(default 'row')\n"
	     << "Options for JSON format:\n"
	     << "\tSTREAM=ON|OFF\tWrite JSON into one stream or format it (ON)\n"
	     << "\tROOT=text\tName for the JSON root key (default 'data')\n"
	     << "\tSETS=text[,text]\tName(s) of the recordset(s), for multiset queries\n"
	     << "\tROWS=text[,text]\tName(s) for row element in recordset(s)(default 'row')\n"
	     << "\tFIELDPERLINE=ON|OFF\tPrint each field on its own line\n"
	     << "\n"
	     << "\nExamples:\n"
	     << "  nisql -v DSN=MyDb1 Table1 out.txt -fHDR=off\n\n"
	     << "  nisql \"DSN=MyDb2;UID=jim;PWD=123\"\n"
	     << "        \"select f1,f5 from sales where customer='mary'\"\n"
	     << "        mary-sales.csv -f \"HDR=on;DELIM=,\"\n"
	     << endl;
}
